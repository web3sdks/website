...
function addAPIKey<T extends string | number>(
  ...
  const entries = (Object.entries(input) as [T, string][]).map(
    ([key, value]) => {
      ...
      if (!url.hostname.includes("web3sdks.com")) {
        return [key, value];
      }
      url.pathname =
