...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    ...
    const sdk = getEVMWeb3sdksSDK(chainId as SUPPORTED_CHAIN_ID);
...
async function fetchConstructorParams(
  ...
  if (
    ...
    try {
      ...
      // contracts not released through web3sdks
    }
    ...
    try {
      ...
      throw new Error(
        "Verifying this contract requires a release. Run `npx web3sdks release` to create a release for this contract, then try again.",
      );
    }
...
async function fetchDeployBytecodeFromReleaseMetadata(
  ...
  if (compialierMetaUri) {
    const pubmeta = await getEVMWeb3sdksSDK(ChainId.Polygon)
      .getPublisher()
